...
# ——— онбординг: Шаг 5 → финал (99) ——————————
    ...
    if await get_onboarding_step(user_id) == 5:
        ...
        await call.message.answer(
            "🎉 Поздравляю, Вы прошли все шаги!\n"
            "Теперь Вы умеете добавлять практики, вести логи, смотреть прогресс и строить графики.\n"
            "Если возникнут вопросы, воспользуйтесь командой /help."
        )
...
async def start_handler(message: Message, state: FSMContext):
    ...
    if step == 0:
        ...
        await message.answer(
            "Шаг 1 / 6. Добро пожаловать в Sadhana_bot! "
            "Этот бот поможет Вам вести дневник практик, анализировать "
            "результаты и поддерживать регулярность.\n\n"
            "Чтобы напоминания приходили вовремя, укажите временную "
            "зону командой /settimezone."
        )
        return
...
async def cb_log_finish(call: CallbackQuery, state: FSMContext):
    ...
    if await get_onboarding_step(user_id) == 3:
        ...
        await call.message.answer(
            "Отлично! Шаг 4/6. Теперь взглянем на общий прогресс.\n"
            "Введите /progress, чтобы посмотреть статистику по дням."
        )
...
async def cb_timezone_select(call: CallbackQuery, state: FSMContext):
    ...
    if step == 1:
        ...
        text += (
            "\n\n<b>Шаг 2 / 6.</b> Отличная работа! "
            "Теперь добавьте свою первую практику командой /addpractice.\n"
            "Это может быть йога, бег, английский язык, рисование — "
            "любое занятие, которое хотите отслеживать."
        )
...
async def show_progress(message: types.Message, state: FSMContext):
    ...
    if await get_onboarding_step(user_id) == 4:
        ...
        await message.answer(
            "Отлично! Шаг 5/6. Теперь построим наглядный график.\n"
            "Введите /chart, чтобы увидеть динамику своих практик."
        )
...
async def save_practice(message: Message, state: FSMContext):
    ...
    if await get_onboarding_step(user_id) == 2:
        ...
        await message.answer(
            "Отлично! Шаг 3/6. Теперь давайте зафиксируем результат.\n"
            "Введите команду /log и отметьте выполненную практику."
        )
    await state.clear()
...
async def set_timezone_handler(message: Message, state: FSMContext):
    ...
    if step == 1:
        await set_onboarding_step(user_id, 2)
        text += (
            "\n\nШаг 2/6. Отличная работа! "
            "Теперь добавьте свою первую практику через /addpractice.\n"
            "Это может быть йога, бег, английский язык, рисование — "
            "любое занятие, которое хотите отслеживать."
        )
